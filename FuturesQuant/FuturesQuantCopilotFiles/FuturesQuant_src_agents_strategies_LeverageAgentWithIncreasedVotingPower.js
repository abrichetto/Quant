/**
 * LeverageAgentWithIncreasedVotingPower
 * 
 * Manages leverage for trades with a 25% increase in voting power.
 * Focuses on maximizing profit while maintaining acceptable risk levels.
 */

import BaseAgent from './BaseAgent';

class LeverageAgentWithIncreasedVotingPower extends BaseAgent {
  constructor(config = {}) {
    super({
      name: 'Leverage Manager with Increased Voting Power',
      description: 'Manages optimal leverage for trades based on consensus and risk with increased voting power',
      ...config
    });
    ...
    this.settings = {
      ...
      votingPowerIncrease: config.votingPowerIncrease || 0.25 // 25% increase
    };
  ...
  analyzeConsensus() {
    ...
    managers.forEach(manager => {
      ...
      totalConfidence += opinion.confidence * (1 + this.settings.votingPowerIncrease);
...
export default LeverageAgentWithIncreasedVotingPower;